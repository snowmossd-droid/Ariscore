...
public class TargetStrafe extends Module {
    ...
    // Velocity mode (được đổi tên từ Collision): orbit quanh target, mượt và ít flag hơn
    ...
    /*
     * Velocity mode: thay vì lao thẳng vào target (dễ bị anti-cheat mạnh như Grim/Vulcan flag
     * vì gia tốc hướng tâm bất thường), ta strafe theo phương tiếp tuyến (vuông góc với
     * hướng tới target) và chỉ hiệu chỉnh nhẹ bán kính để giữ khoảng cách. Chuyển động được
     * blend với vận tốc hiện tại (smoothing) nên không có bước nhảy vận tốc đột ngột.
     */
    private void handleVelocityMode(ClientPlayerEntity player) {
        ...
        // Vùng đệm (hysteresis): không chỉnh khi đã ở trong khoảng hợp lệ -> tránh giật/rung bị flag
        double tolerance = 0.35;

        // Hướng tới target (chuẩn hóa) và phương tiếp tuyến (vuông góc)
        double toX = dx / dist;
        double toZ = dz / dist;
        double tanX = this.clockwise ? -toZ : toZ;
        double tanZ = this.clockwise ? toX : -toX;

        // Thành phần hiệu chỉnh bán kính, mượt theo độ lệch khoảng cách
        double radial = 0.0;
        if (dist > desired + tolerance) {
            radial = Math.min(1.0, (dist - desired) / 2.0); // tiến vào, càng xa càng nhanh nhưng có cap
        } else if (dist < desired - tolerance) {
            radial = -Math.min(1.0, (desired - dist) / 2.0); // lùi ra
        }

        double speed = this.velocitySpeed.getValue();
        // Vector mong muốn = tiếp tuyến (chủ đạo) + thành phần hướng tâm nhẹ
        double wishX = (tanX + toX * radial) * speed;
        double wishZ = (tanZ + toZ * radial) * speed;

        // Blend với vận tốc hiện tại để chuyển động liền mạch (giảm gia tốc đột ngột)
