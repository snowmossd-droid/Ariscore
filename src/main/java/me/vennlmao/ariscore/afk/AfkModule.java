...
import me.vennlmao.ariscore.afk.commands.DelAfkCommand;
...
public class AfkModule {
    ...
    public void enable() {
        if (!new LicenseManager(plugin).validate()) {
            plugin.getLogger().severe("[ArisCore] AfkModule disabled: invalid license.");
            return;
        }
        loadConfig();
        ...
        DelAfkCommand delAfkCmd = new DelAfkCommand(this);
        plugin.getCommand("delafk").setExecutor(delAfkCmd);
        plugin.getCommand("delafk").setTabCompleter(delAfkCmd);
