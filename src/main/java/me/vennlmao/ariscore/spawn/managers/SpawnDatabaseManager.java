...
public class SpawnDatabaseManager implements AutoCloseable {

    private final SpawnModule module;
    private HikariDataSource dataSource;
    private boolean mysql;

    public SpawnDatabaseManager(SpawnModule module) {
        this.module = module;
    }
    ...
    private String tableName() {
        return prefix() + module.getConfig().getString("mysql.table-name", "spawns");
    }
    ...
    public void init() {
        ...
        if (mysql) {
            ...
            File dbFile = new File(module.getPlugin().getDataFolder(),
                    "spawn/" + module.getConfig().getString("mysql.table-name", "spawns") + ".db");
        ...
        config.setPoolName("ArisSpawn-Pool");
    ...
    private void createTable() {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            module.getPlugin().getLogger().severe("[Spawn] Failed to create table: " + e.getMessage());
        }
    }

    public Map<String, Location> getAllSpawns() {
        Map<String, Location> spawns = new LinkedHashMap<>();
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            while (rs.next()) {
                ...
                spawns.put(name, new Location(world,
                        rs.getDouble("x"), rs.getDouble("y"), rs.getDouble("z"),
                        rs.getFloat("yaw"), rs.getFloat("pitch")));
            }
        } catch (SQLException e) {
            module.getPlugin().getLogger().severe("[Spawn] Failed to get spawns: " + e.getMessage());
        }
        return spawns;
    }
    ...
    public void deleteSpawn(String name) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            module.getPlugin().getLogger().severe("[Spawn] Failed to delete spawn: " + e.getMessage());
        }
    }
    ...
    public void setSpawn(String name, Location loc) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            module.getPlugin().getLogger().severe("[Spawn] Failed to set spawn: " + e.getMessage());
        }
    }
