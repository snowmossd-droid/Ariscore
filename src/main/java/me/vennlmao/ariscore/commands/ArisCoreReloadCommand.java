...
public class ArisCoreReloadCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        switch (module) {
            ...
            case "warp"   -> { plugin.getWarpModule().reload();   sender.sendMessage(MM.deserialize("<green>Warp module reloaded.")); }
            case "crates" -> { plugin.getCratesModule().reload(); sender.sendMessage(MM.deserialize("<green>Crates module reloaded.")); }
            default -> {
                ...
                plugin.getWarpModule().reload();
                plugin.getCratesModule().reload();
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command command,
                                      @NotNull String label, @NotNull String[] args) {
        if (args.length == 1) {
            return Arrays.asList("all", "tpa", "home", "shop", "shards", "team", "auction", "sell", "tab", "order", "spawn", "afk", "warp", "crates").stream()
