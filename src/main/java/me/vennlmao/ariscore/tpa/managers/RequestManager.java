...
public class RequestManager {
    ...
    // Request đã gửi đến người nhận (active)
    private final Map<UUID, TpaRequest> pendingRequests = new HashMap<>();

    // Request đang chờ sender confirm qua GUI (chưa gửi)
    private final Map<UUID, TpaRequest> preSendRequests = new HashMap<>();
    ...
    // ── Active requests (đã gửi đến receiver) ──
    ...
    public TpaRequest getRequestBySender(Player sender) {
        for (TpaRequest r : pendingRequests.values()) {
            if (r.getSender().getUniqueId().equals(sender.getUniqueId())) return r;
        }
        return null;
    }
    ...
    // ── Pre-send requests (chờ sender confirm GUI trước khi gửi) ──

    public void addPreSendRequest(TpaRequest request) {
        preSendRequests.put(request.getSender().getUniqueId(), request);
    }

    public TpaRequest getPendingSenderRequest(Player sender) {
        return preSendRequests.get(sender.getUniqueId());
    }

    public void removePendingSenderRequest(Player sender) {
        preSendRequests.remove(sender.getUniqueId());
    }

    // Sender confirm GUI → chuyển từ pre-send sang active
    public void promotePendingToActive(Player sender) {
        TpaRequest request = preSendRequests.remove(sender.getUniqueId());
