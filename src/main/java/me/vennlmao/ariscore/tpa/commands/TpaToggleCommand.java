...
public class TpaToggleCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!nowDisabled) {
            ...
            // Hủy request TPA đang pending, thông báo cho sender
            TpaRequest pending = plugin.getRequestManager().getRequest(player);
            if (pending != null && pending.getType() == TpaRequest.Type.TPA) {
                Player requester = pending.getSender();
                plugin.getRequestManager().removeRequest(player);
                if (requester.isOnline()) {
                    MessageUtil.sendChatList(requester, "target_tpa_disabled",
                            s -> s.replace("{player}", player.getName()));
                    MessageUtil.sendActionbar(requester, "target_tpa_disabled_ab",
                            s -> s.replace("{player}", player.getName()));
