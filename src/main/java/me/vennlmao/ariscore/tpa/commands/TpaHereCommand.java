...
public class TpaHereCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (args.length == 0) { SoundUtil.play(player, "error"); return true; }
        ...
        if (target.equals(player)) { SoundUtil.play(player, "error"); return true; }
        ...
        MessageUtil.sendActionbar(target, "request_received_tpahere_ab",
                s -> s.replace("{player}", player.getName()));
    ...
    private void sendClickableRequest(Player receiver, Player requester) {
        List<String> lines = plugin.getConfig().getStringList("clickable_messages.accept_tpahere.text");
        for (String line : lines) {
            ...
            Component component = MM.deserialize("<!italic>" + converted)
                    .clickEvent(ClickEvent.runCommand("/tpahereaccept " + requester.getName()));
