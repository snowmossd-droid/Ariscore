    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "sell": {
                ...
                if (!player.hasPermission("ariscore.auction.sell")) { player.sendMessage(lang.getNoPermission()); return true; }
                if (args.length < 2) { player.sendMessage(lang.getHelp().toString()); return true; }

                ItemStack hand = player.getInventory().getItemInMainHand();
                if (hand == null || hand.getType().isAir()) { player.sendMessage(lang.getNoItemInHand()); return true; }

                Double price = EcoUtil.parsePrice(args[1], cfg);
                if (price == null) { player.sendMessage(lang.getInvalidPrice()); return true; }

                if (cfg.isBlacklisted(hand.getType())) {
                    player.sendMessage(lang.getBlacklistedItem(lang.formatItemName(hand)));
            ...
            case "view": {
                ...
                if (target == null) { player.sendMessage(lang.getItemNotFound()); return true; }
            ...
            case "reload": {
                ...
                if (sender instanceof Player) cfg.playSound((Player) sender, "reload");
                return true;
            }
