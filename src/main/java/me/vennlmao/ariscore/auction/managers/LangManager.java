...
public class LangManager {
    ...
    public boolean isChatEnabled() { return lang.getBoolean("display.chat-enable", true); }
    public boolean isActionbarEnabled() { return lang.getBoolean("display.actionbar-enable", true); }
    ...
    public void send(Player player, String built) {
        if (built == null || built.isEmpty()) return;
        if (isChatEnabled()) player.sendMessage(built);
        if (isActionbarEnabled()) player.sendActionBar(toActionbarComponent(built));
    }

    public void send(Player player, String key, String... replacements) {
        send(player, msg(key, replacements));
    }
