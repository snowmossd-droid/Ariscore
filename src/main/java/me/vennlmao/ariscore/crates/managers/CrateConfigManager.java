...
public class CrateConfigManager {
    ...
    public void addItemToReward(String crateName, String rewardId, ItemStack item) {
        File file = getCrateFile(crateName);
        ...
        if (cfg.getConfigurationSection(path) == null) {
            cfg.set(path + ".slot", nextFreeSlot(cfg));
        ...
        List<Object> itemList = new ArrayList<>(cfg.getMapList(path + ".items"));
    ...
    private int nextFreeSlot(FileConfiguration cfg) {
        ...
        if (rewardsSection != null) {
            for (String key : rewardsSection.getKeys(false)) {
                ConfigurationSection s = rewardsSection.getConfigurationSection(key);
                if (s != null) used.add(s.getInt("slot", -1));
            }
        }
        for (int i = 0; i < 54; i++) {
