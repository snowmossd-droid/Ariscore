...
public class TeamManager {
    ...
    public boolean createTeam(Player owner, String name) {
        ...
        team.addMember(owner.getUniqueId(), TeamData.Role.OWNER);
        teams.put(name.toLowerCase(), team);
        playerTeam.put(owner.getUniqueId(), name);

        db.saveTeam(team);
        db.saveMember(owner.getUniqueId(), name, TeamData.Role.OWNER);
        return true;
    }
    ...
    public boolean addMember(String teamName, Player player) {
        ...
        team.addMember(player.getUniqueId(), TeamData.Role.MEMBER);
        playerTeam.put(player.getUniqueId(), teamName);
        db.saveMember(player.getUniqueId(), teamName, TeamData.Role.MEMBER);
        return true;
    }
    ...
    public void promoteToCoOwner(UUID uuid) {
        TeamData team = getPlayerTeam(uuid);
        if (team == null) return;
        team.addMember(uuid, TeamData.Role.CO_OWNER);
        db.saveMember(uuid, team.getName(), TeamData.Role.CO_OWNER);
    }

    public void demoteToMember(UUID uuid) {
        TeamData team = getPlayerTeam(uuid);
        if (team == null) return;
        team.addMember(uuid, TeamData.Role.MEMBER);
        db.saveMember(uuid, team.getName(), TeamData.Role.MEMBER);
    }
    ...
    public TeamData getPlayerTeam(UUID uuid) {
        ...
        return name != null ? getTeam(name) : null;
    }
    ...
    public void addInvite(UUID invitedUuid, UUID teamOwnerUuid) {
        pendingInvites.computeIfAbsent(invitedUuid, k -> new HashSet<>()).add(teamOwnerUuid);
    }
    ...
    public boolean hasInvite(UUID uuid, String teamName) {
        ...
        TeamData team = getTeam(teamName);
        if (team == null) return false;
        return invites.stream().anyMatch(ownerUuid -> {
            TeamData ownerTeam = getPlayerTeam(ownerUuid);
            return ownerTeam != null && ownerTeam.getName().equalsIgnoreCase(teamName);
        });
    }
    ...
    public String getInviteTeam(UUID uuid) {
        ...
        UUID ownerUuid = invites.iterator().next();
        TeamData team = getPlayerTeam(ownerUuid);
