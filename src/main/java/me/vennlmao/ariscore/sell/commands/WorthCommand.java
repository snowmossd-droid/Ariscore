...
public class WorthCommand implements CommandExecutor {
    ...
    public WorthCommand(SellModule module) {
        this.module = module;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("ariscore.sell.worth")) {
            player.sendMessage(ColorUtil.colorize(module.getConfig().getString("no-permission", "&cYou do not have permission to use this command!")));
            return true;
        }
        player.openInventory(new WorthMenu(module).getInventory());
        return true;
    }
}
