...
public class PapiManager {
    ...
    private static String gradient(String original, String stripped, String startHex, String endHex) {
        ...
        // Thu thập tất cả format codes (&l &o &n &m &k) từ original
        // để apply lại sau mỗi hex color (vì hex reset format)
        StringBuilder fmtBuilder = new StringBuilder();
        for (int i = 0; i < original.length() - 1; i++) {
            if (original.charAt(i) == '&') {
                char next = original.charAt(i + 1);
                if ("klmnorKLMNOR".indexOf(next) >= 0) {
                    fmtBuilder.append('§').append(Character.toLowerCase(next));
                    i++;
                }
            }
        }
